...
<%
    ...
    if (request.getAttribute("data") != null) {
        data = (ArrayList<OrderDetail>) request.getAttribute("data");
         for (OrderDetail c : data) {
        // Parse price and discount once
        double price = Double.parseDouble(c.getCourse().getPrice());
        double discount = Double.parseDouble(c.getCourse().getDiscount());
...
%>
                                        ...
                                        <c:forEach items="${data}" var="c">
                                            <tr>
                                                ...
                                                <td>${c.getCourse().getTitle()}</td>
                                                <td>
                                                    <a href="#" data-toggle="modal" data-target="#imageModal" data-image="${c.getCourse().getImgUrl()}">
                                                        <img class="img-fluid" style="max-width: 100px; text-align: center" src="${c.getCourse().getImgUrl()}" alt="image"/>
                                                    </a>
                                                </td>
                                                <td>${c.getCourse().getNameTeacher()}</td>
                                                <td>${c.getCourse().getStart()}</td>
                                                <td>${c.getCourse().getDuration()}</td>
                                                <td>${c.getCourse().getPrice()}</td>
                                                <td>${c.getCourse().getDiscount()}</td>
                                                <td>${c.getCourse().getPrice()*(1-c.getCourse().getDiscount())}</td>
