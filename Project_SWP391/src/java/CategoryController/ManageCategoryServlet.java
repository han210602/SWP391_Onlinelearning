...
//        }
    ...
    throws ServletException, IOException {
        ...
        try {
            ...
            //If id_raw not null , Update category
            if (!"".equals(id_raw)) { 
                id = Integer.parseInt(id_raw);
                cd.updateCategory(name_raw,description_raw,id);
                response.sendRedirect("categorylist");
            }
            //Create a category
            else {
                //check if category with name is exist(id is identity). If not, create category
                Category category = cd.getCategoryByName(name_raw); 
                if (category == null) {
                    cd.createCategory(name_raw, description_raw);
                    response.sendRedirect("categorylist");
                } else {
